...
def convert_one(tif_path: Path, label: str):
    """Lit un GeoTIFF, retourne (image_PIL_avec_bandeau, geo_dict)."""
    ...
    header_h = int(new_h * 0.07)
    canvas = Image.new("RGB", (new_w, new_h + header_h), BRAND)
    canvas.paste(img_resized, (0, header_h))
    d = ImageDraw.Draw(canvas)
    try:
        font = ImageFont.truetype(
            "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf",
            max(14, int(header_h * 0.45)),
        )
    except OSError:
        font = ImageFont.load_default()
    d.text((14, header_h * 0.22), label, font=font, fill=MIST)
